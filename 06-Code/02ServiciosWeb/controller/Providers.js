...
const updateProvider = (req, res) => {
        Provider.findOneAndUpdate({id: req.body.id},{total: req.body.total},(err,prov) =>{
            err && res.status(500).send(err.message)
...
const deleteProvider = (req, res) => {
    Provider.findOneAndDelete({id: req.body.id},(err,prov) => {
        err && res.status(501).send(err.message)
        res.status(200).send(prov)
    })

}
module.exports = {createProvider, getProviders, updateProvider, deleteProvider}